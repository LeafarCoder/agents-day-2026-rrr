...
ALTER TABLE user_preferences
    ADD COLUMN activity_keyword_id uuid REFERENCES activity_keywords (id) ON DELETE RESTRICT;

DROP INDEX IF EXISTS idx_user_preferences_preference_id;

CREATE UNIQUE INDEX idx_user_preferences_user_keyword
    ON user_preferences (user_id, activity_keyword_id);

CREATE INDEX idx_user_preferences_keyword_id
