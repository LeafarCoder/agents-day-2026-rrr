...
export default function PreferencesPage() {
  ...
  // Which category is showing the inline keyword input
  ...
  useEffect(() => {
    if (addingTo) kwInputRef.current?.focus()
  }, [addingTo])
  ...
  async function deleteKeyword(category: string, keyword: string) {
    if (!confirm(`Delete keyword "${keyword}" from ${category.replace(/_/g, ' ')}?`)) return
    setError(null)
    try {
      const r = await fetch(`${API_URL}/api/preferences/keywords?category=${encodeURIComponent(category)}&keyword=${encodeURIComponent(keyword)}`, {
  ...
  async function deleteCategory(category: string) {
    if (!confirm(`Delete category "${category.replace(/_/g, ' ')}" and all its custom keywords?`)) return
    setError(null)
    try {
                  ...
                  {!isDemo && !isDefault && (
                    <button
                      ...
                      style={{
                        ...
                        opacity: 0.6, transition: 'opacity 150ms, color 150ms',
                        lineHeight: 1,
                      }}
